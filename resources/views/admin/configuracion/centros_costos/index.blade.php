...
@section('title', 'Centros de Costos')

@section('header', 'Gestión de Centros de Costos')
            ...
            <div class="card-body p-4">
                <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4">
                    <div class="mb-3 mb-sm-0">
                        ...
                        <p class="text-muted small mb-0">Gestiona los centros de costos desde aquí</p>
                    </div>
                    <div>
                        <a href="{{ route('admin.configuracion.centros_costos.create') }}" class="btn btn-primary d-flex align-items-center">
                        ...
                        <tbody>
                            @forelse ($centros as $index => $centro)
                                <tr>
                                    ...
                                    <td class="fw-medium">{{ $centro->codigo }}</td>
                                    <td>{{ $centro->nombre }}</td>
                                    <td>{{ $centro->descripcion ?? 'Sin descripción' }}</td>
                                    <td class="text-end">
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('admin.configuracion.centros_costos.edit', $centro) }}" class="btn btn-outline-warning" title="Editar">
                                            ...
                                            <form action="{{ route('admin.configuracion.centros_costos.destroy', $centro) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('¿Estás seguro de eliminar este centro de costo?')" class="btn btn-outline-danger" title="Eliminar">
                                                    <svg xmlns="http://www.w3.org/2000/svg" style="height: 1rem; width: 1rem;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <tr>
                                    <td colspan="5" class="py-5 text-center">
                                        <div class="d-flex flex-column align-items-center justify-content-center">
                                            ...
                                            <p class="text-muted mb-2">No hay centros de costos registrados</p>
                                            <a href="{{ route('admin.configuracion.centros_costos.create') }}" class="btn btn-sm btn-link text-decoration-none">
