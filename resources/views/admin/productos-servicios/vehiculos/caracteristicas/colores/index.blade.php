<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4">
    <div class="mb-3 mb-sm-0">
        <h2 class="h4 fw-bold mb-1" :class="darkMode ? 'text-light' : 'text-dark'">
            Total de Colores: {{ $colores->total() }}
        </h2>
        <p class="text-muted small mb-0">Gestiona los colores de vehículos desde aquí</p>
    </div>
    <div>
        <a href="{{ route('admin.productos-servicios.vehiculos.caracteristicas.colores.create') }}" class="btn btn-primary d-flex align-items-center">
    ...
    <table class="table table-hover align-middle" :class="darkMode ? 'table-dark' : ''">
        ...
        <tbody>
            @forelse ($colores as $index => $color)
                <tr>
                    <td>{{ $colores->firstItem() + $index }}</td>
                    <td>{{ $color->nombre }}</td>
                    <td>
                        <div style="width: 30px; height: 30px; background-color: {{ $color->hexadecimal }}; border: 1px solid #ccc; display: inline-block;"></div>
                        {{ $color->hexadecimal }}
                    </td>
                    <td class="text-end">
                        <div class="btn-group btn-group-sm">
                            <a href="{{ route('admin.productos-servicios.vehiculos.caracteristicas.colores.edit', $color) }}" class="btn btn-outline-warning" title="Editar">
                            ...
                            <form action="{{ route('admin.productos-servicios.vehiculos.caracteristicas.colores.destroy', $color) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('¿Estás seguro de eliminar este color?')" class="btn btn-outline-danger" title="Eliminar">
                                    <svg xmlns="http://www.w3.org/2000/svg" style="height: 1rem; width: 1rem;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <tr>
                    <td colspan="4" class="py-5 text-center">
                        <div class="d-flex flex-column align-items-center justify-content-center">
                            ...
                            <p class="text-muted mb-2">No hay colores registrados</p>
                            <a href="{{ route('admin.productos-servicios.vehiculos.caracteristicas.colores.create') }}" class="btn btn-sm btn-link text-decoration-none">
