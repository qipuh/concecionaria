<div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4">
    <div class="mb-3 mb-sm-0">
        ...
        <p class="text-muted small mb-0">Gestiona las marcas de vehículos desde aquí</p>
    </div>
    <div>
        <a href="{{ route('admin.productos-servicios.vehiculos.caracteristicas.marcas.create') }}" class="btn btn-primary d-flex align-items-center">
    ...
    <table class="table table-hover align-middle" :class="darkMode ? 'table-dark' : ''">
        ...
        <tbody>
            @forelse ($marcas as $index => $marca)
                <tr>
                    <td>{{ $marcas->firstItem() + $index }}</td>
                    <td>{{ $marca->nombre }}</td>
                    <td class="text-end">
                        <div class="btn-group btn-group-sm">
                            <a href="{{ route('admin.productos-servicios.vehiculos.caracteristicas.marcas.edit', $marca) }}" class="btn btn-outline-warning" title="Editar">
                            ...
                            <form action="{{ route('admin.productos-servicios.vehiculos.caracteristicas.marcas.destroy', $marca) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('¿Estás seguro de eliminar esta marca?')" class="btn btn-outline-danger" title="Eliminar">
                                    <svg xmlns="http://www.w3.org/2000/svg" style="height: 1rem; width: 1rem;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <tr>
                    <td colspan="3" class="py-5 text-center">
                        <div class="d-flex flex-column align-items-center justify-content-center">
                            ...
                            <p class="text-muted mb-2">No hay marcas registradas</p>
                            <a href="{{ route('admin.productos-servicios.vehiculos.caracteristicas.marcas.create') }}" class="btn btn-sm btn-link text-decoration-none">
