...
@section('header', 'Registro de Partes')
            ...
            <div class="card-body p-4">
                <div class="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4">
                    <div class="mb-3 mb-sm-0">
                        ...
                        <p class="text-muted small mb-0">Gestiona las partes desde aquí</p>
                    </div>
                    <div>
                        <a href="{{ route('admin.almacenes.partes.create') }}" class="btn btn-primary d-flex align-items-center">
                    ...
                    <table class="table table-hover align-middle" :class="darkMode ? 'table-dark' : ''">
                        <thead class="table-light" :class="darkMode ? 'table-dark' : ''">
                            <tr>
                                ...
                                <th scope="col" class="text-uppercase small">Unidad</th>
                                <th scope="col" class="text-uppercase small">Fabricante</th>
                                <th scope="col" class="text-uppercase small">Proveedor</th>
                                <th scope="col" class="text-uppercase small">Categoría</th>
                                <th scope="col" class="text-uppercase small">Precio Venta</th>
                        ...
                        <tbody>
                            @forelse ($partes as $index => $parte)
                                <tr>
                                    <td>{{ $partes->firstItem() + $index }}</td>
                                    <td>{{ $parte->codigo }}</td>
                                    <td>{{ $parte->nombre }}</td>
                                    <td>{{ $parte->unidad->nombre ?? 'N/A' }}</td>
                                    <td>{{ $parte->fabricante->nombre_fabricante ?? 'N/A' }}</td>
                                    <td>{{ $parte->proveedor ? $parte->proveedor->nombre_completo : 'N/A' }}</td>
                                    <td>{{ $parte->categoriaParte->nombre ?? 'N/A' }}</td>
                                    <td>{{ number_format($parte->precio_venta, 2) }} {{ $parte->moneda_venta }}</td>
                                    <td class="text-end">
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('admin.almacenes.partes.edit', $parte) }}" class="btn btn-outline-warning" title="Editar">
                                            ...
                                            <form action="{{ route('admin.almacenes.partes.destroy', $parte) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('¿Estás seguro de eliminar esta parte?')" class="btn btn-outline-danger" title="Eliminar">
                                                    <svg xmlns="http://www.w3.org/2000/svg" style="height: 1rem; width: 1rem;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <tr>
                                    <td colspan="8" class="py-5 text-center">
                                        <div class="d-flex flex-column align-items-center justify-content-center">
                                            ...
                                            <p class="text-muted mb-2">No hay partes registradas</p>
                                            <a href="{{ route('admin.almacenes.partes.create') }}" class="btn btn-sm btn-link text-decoration-none">
