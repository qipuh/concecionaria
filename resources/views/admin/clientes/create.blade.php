        ...
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center">
            <div class="mb-3 mb-lg-0">
                <div class="d-inline-flex align-items-center px-3 py-1 bg-white bg-opacity-10 rounded-pill fs-6 mb-3 border border-white border-opacity-25">
                    <i class="fas fa-user-plus text-info me-2"></i> Clientes
                </div>
                <h2 class="fw-bold mb-1 tracking-tight text-white display-6 text-shadow-sm">
                    Nuevo Cliente
                </h2>
                <p class="text-white-50 mb-0">Registra los datos para crear un nuevo cliente.</p>
            </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <input type="text" name="documento_identidad" id="documento_identidad"
                                   ...
                                   value="{{ old('documento_identidad') }}" required maxlength="11" autocomplete="off">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="email" name="correo" id="correo"
                                   ...
                                   placeholder="cliente@dominio.com" value="{{ old('correo') }}">
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="categoria_cliente_id" id="categoria_cliente_id"
                            ...
                            @foreach ($categorias as $categoria)
                                <option value="{{ $categoria->id }}" {{ old('categoria_cliente_id') == $categoria->id ? 'selected' : '' }}>
                                    {{ $categoria->nombre }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="d-flex flex-wrap gap-2 mt-1">
                            @foreach ($canales as $canal)
                                <input type="radio" class="btn-check" name="canal_captacion_id"
                                       ...
                                       {{ old('canal_captacion_id') == $canal->id ? 'checked' : '' }} required>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function limpiarCamposDocumento() {
        ['documento_identidad','apellido_paterno','apellido_materno','nombres','razon_social'].forEach(id => {
    ...
    if (tipoClienteSelect.value) mostrarCamposSegunTipo();

    // Auto-validación al escribir (debounce)
    let timeoutId = null;
    ...
    documentoInput.addEventListener('input', function (e) {
        ...
        if (tipo === 'natural'  && valor.length === 8)  {
            documentoHint.innerHTML = '<i class="fas fa-check text-success"></i> DNI válido — buscando datos...';
            timeoutId = setTimeout(() => autoValidar(), 800);
        } else if (tipo === 'juridica' && valor.length === 11) {
            documentoHint.innerHTML = '<i class="fas fa-check text-success"></i> RUC válido — buscando datos...';
            timeoutId = setTimeout(() => autoValidar(), 800);
        } else if (valor.length > 0) {
            const esperado = tipo === 'natural' ? 8 : 11;
            const tipoLabel = tipo === 'natural' ? 'DNI' : 'RUC';
            documentoHint.innerHTML = `<i class="fas fa-info-circle text-info"></i> ${tipoLabel}: ${valor.length}/${esperado} dígitos`;
        } else {
            documentoHint.textContent = tipo === 'natural' ? 'Ingrese 8 dígitos del DNI' : 'Ingrese 11 dígitos del RUC';
        }
    });
    ...
    function autoValidar() {
        ...
        const tipo = tipoClienteSelect.value;
        const numero = documentoInput.value;
        const expected = tipo === 'natural' ? 8 : 11;
        if (!tipo || numero.length !== expected) return;
        validarAPI(tipo === 'natural' ? 'DNI' : 'RUC', numero, true);
    }

    function validarAPI(tipoDoc, numeroDoc, esAuto = false) {
        if (isValidating) return;
        isValidating = true;
        ...
        if (esAuto) {
            ...
            documentoHint.innerHTML = '<i class="fas fa-spinner fa-spin text-primary"></i> Consultando...';
        } else {
            btn.disabled = true;
        ...
        .then(data => {
            if (data.success) {
                procesarDatos(data.data, tipoDoc, esAuto);
                documentoHint.innerHTML = `<i class="fas fa-check text-success"></i> ${esAuto ? 'Datos encontrados automáticamente' : 'Validación exitosa'}`;
            } else {
                if (esAuto) {
        ...
        .catch(() => {
            if (esAuto) documentoHint.innerHTML = '<i class="fas fa-times text-danger"></i> Error en la consulta';
            else alert('Error al validar el documento. Intente nuevamente.');
        })
        .finally(() => {
            isValidating = false;
    ...
    function procesarDatos(data, tipoDoc, esAuto) {
        if (tipoDoc === 'DNI') {
            ...
            if (esAuto) {
                ['apellido_paterno','apellido_materno','nombres'].forEach(id => document.getElementById(id).readOnly = false);
            }
        } else {
            ...
            if (esAuto) document.getElementById('razon_social').readOnly = false;
            if (data.departamento) procesarUbicacion(data);
        }
    }
    ...
    function procesarUbicacion(data) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => { document.getElementById('distrito').value = data.distrito || ''; }, 600);
        }, 600);
    }
    ...
    document.getElementById('validar_documento').addEventListener('click', function () {
        ...
        validarAPI(tipo === 'natural' ? 'DNI' : 'RUC', numero, false);
    });

    // Cargar provincias/distritos
    ...
    // Pre-cargar si hay valores old
    const depSel = document.getElementById('departamento');
    const provSel = document.getElementById('provincia');
    if (depSel.value)  depSel.dispatchEvent(new Event('change'));
